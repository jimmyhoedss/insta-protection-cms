    ...
    public function actionUpdate($id) {
        ...
        if ($modelAction->load(Yii::$app->request->post()) && $modelPlanDetail2->load(Yii::$app->request->post()) && $planActionForm->load(Yii::$app->request->post())) {
            ...
            try{
                ...
                $caseStatus = InstapPlanPool::mapActionToStatus()[$modelAction->action_status];
                //UserPlan model redundancy
                $model->plan_status = $modelPlan->current_plan_status = $caseStatus;

                if ($planActionForm->validate() && $modelAction->save() && $model->save() && $modelPlan->save()) {
                    //retrive plan_action id first then save document
                    if($modelAction->action_status === UserPlanAction::ACTION_PHYSICAL_ASSESSMENT || $modelAction->action_status === UserPlanAction::ACTION_UPLOAD_PHOTO) {
