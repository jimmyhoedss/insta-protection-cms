...
class InstapReportController extends Controller
{
    ...
    public function actionReport()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if($useDataProvider){
                ...
                if($folder_name){
                    $directory = '@backend/web/report/'.$model->today." ".$model->region_id;
                    ...
                    chmod(Utility::replacePathAccordingToOS($zip_file_name), 0755);
                    if($successZip){
                        if($this->postDocument($zip_file_name, 'zip')) {
                            ...
                            if($model->save()) {
                                try {
                                    FileHelper::removeDirectory(Utility::replacePathAccordingToOS(Yii::getAlias($directory)));
                                } catch (\Exception $e) {
                                    \Yii::warning(json_encode($e), "FileHelper::Exception");
                                }
                                Yii::$app->session->setFlash('success', "Report generate successful, click <i class='fa fa-link'></i> to download.");   
                            } else {
                                Yii::$app->session->setFlash('error', "Report generate fail");
    ...
    public static function zipDir($sourcePath, $outZipPath){
        // try {
            ...
            //note: ZipArchive not found when deploy to aws server.
            //do the following:
