...
<div class="dealer-order-inventory-overview-view">
    ...
    <?php
        ...
        foreach($models as $m) {
            ...
            if($m->action == DealerInventoryAllocationHistory::ACTION_REVERT_ACTIVATE) {
                $html .= "<i>" . $date . "&nbsp; :  " .$from_company . "&nbsp; revert ".$amount." of activated".$plan."<br>"; 
            }

            if($m->action == DealerInventoryAllocationHistory::ACTION_REVERT_ALLOCATE) {
                $html .= "<i>" . $date . "&nbsp; :  Revert ".$amount." of ".$plan." from ".$from_company ." to ".$to_company."<br>"; 
            }
        }
        $html .= "</div>";
