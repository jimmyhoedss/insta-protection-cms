...
class CleanAssetsController extends Controller
{
    ...
    private function setAssetDirs() 
    {
        if ($this->structure == 'basic') {
            ...
            array_push($this->dirs, Yii::getAlias('@api') . '/web/assets');
            array_push($this->dirs, Yii::getAlias('@backend') . '/web/assets');
            array_push($this->dirs, Yii::getAlias('@frontend') . '/web/assets');
        }
    }
