...
class DealerOrderInventory extends MyCustomActiveRecord
{
    ...
    public static function retrieveAllAvailableStock($plan_id, $dealer_company_id, $limit = null){

        $query =  SELF::find()->andWhere(['plan_id' => $plan_id])->andWhere(['dealer_company_id' => $dealer_company_id])->andWhere(['plan_pool_id' => null])->andWhere(['or', ['activation_token'=>null], ['<','expire_at',time()]])->andWhere(['status' => MyCustomActiveRecord::STATUS_ENABLED]);
