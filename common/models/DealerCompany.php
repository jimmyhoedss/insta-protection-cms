...
class DealerCompany extends MyCustomActiveRecord
{
    ...
    public function invInfoByCategory($category) {
        ...
        $plans = $m->getActivePlanByCategory($category)->asArray()->all();
        ...
        foreach($plans as $p) {
            ...
            if($allocate) {
                ...
                array_push($arr, DealerOrderInventoryOverview::showEmpty($p['plan']));
            }
        }
        $o->inventory = $arr;
   ...
   /* public static function listAllDealer(){
    ...
    public static function grandChildren($company_arr, $parent_id = 0) {
        $branch = [];
        foreach ($company_arr as $company) {
