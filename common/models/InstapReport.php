...
class InstapReport extends MyCustomActiveRecord
{
    ...
    public static function getDeclarationColumn() {
        ...
        return[
                    ...
                    [
                        ...
                        'format' => 'raw',
                        'value' => function($model) {  
                            $m = UserPlanAction::find()->select(['created_at'])->andWhere(['action_status' => UserPlanAction::ACTION_APPROVE])->andWhere(['plan_pool_id' => $model->id])->orderBy(['created_at' => SORT_DESC])->asArray()->one();
    ...
    public static function getPlanPoolIdByReportType($report_type, $start_date, $end_date) {
        $plan_pool_id_arr = "";
        switch($report_type) {
            //plan calculated based on approved date
                case self::TYPE_DECLARATION_REPORT:  
                    $approved_plan = UserPlanAction::find()->select(['plan_pool_id','created_at'])->andWhere(['action_status' => UserPlanAction::ACTION_APPROVE])->andWhere(['between', 'created_at', $start_date, $end_date])->orderBy(['created_at' => SORT_DESC])->distinct()->all();
    ...
    public function generateSohReport(){
        ...
        foreach ($this->getMasterDistributorIDs() as $distributor_id) {
            ...
            //remove all processed id with querySOH
    ...
    private function queryAmTransactionReport($distrubutor_company_id, $retailer_company_id, $folder_name){
        ...
        if(count($result) > 0){
            ...
            fclose($file);
            chmod(Utility::replacePathAccordingToOS($file_name), 0755);
        }
    }
    ...
    private function querySohReport($current_dealer_company_id, $downline_company_ids, $folder_name){
        ...
        $downline_companies_condition = ($downline_company_ids == "()") ? "" :"OR a.dealer_company_id IN (SELECT dealer_company_downline_id FROM instaprotection.dealer_company_dealer where dealer_company_upline_id = :current_dealer_company_id)";
        ...
        ";
        $result = \Yii::$app->db->createCommand($query)
        ->bindValue(":current_dealer_company_id", (int) $current_dealer_company_id)
        ->bindValue(":date_end", $this->date_end)
        ->queryAll();
