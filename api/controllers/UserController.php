...
class UserController extends RestControllerBase
{
    ...
    public function beforeAction($action) {
        $isMaintenance = Yii::$app->keyStorage->get(\common\models\KeyStorageItem::APP_MAINTENANCE_MODE);
        if($isMaintenance == \common\components\MyCustomActiveRecord::STATUS_ENABLED){
            $msg = Yii::$app->keyStorage->get(\common\models\KeyStorageItem::APP_MAINTENANCE_MESSAGE);
            throw new \yii\web\BadRequestHttpException(\common\components\Utility::jsonifyError("server", $msg));
        }
        ...
        return array_merge(parent::behaviors(), [
            ...
            'verbs' => [
                ...
                'actions' => [
                    ...
                    'register-plan' => ['POST'],
                    'register-partially' => ['POST'],
                    'plans' => ['GET'],
                    'add-plan' => ['POST'],
                    'my-plans' => ['GET'],
                    'my-plans-by-category' => ['GET'],
            ...
            'rateLimiter' => [
                'class' => \api\components\CustomRateLimiter::className(),
                'enableRateLimitHeaders' => false,
                'only' => ['get-otp'],
                // not needed as authorize and access-token dont have authenticator, so cant track which user is calling api
                //'except' => ['authorize', 'access-token'],
            ],
        ]);       
    }
    ...
    public function actionAddPlan() {
        ...
        if (!isset($this->request['channel']) || !isset($this->request['activation_token'])) {
            throw new CustomHttpException(Utility::jsonifyError("channel", Yii::t('common', "Cannot add plan.")), CustomHttpException::UNPROCESSABLE_ENTITY);
        }
    ...
    //api to support app version before re-skinning
    public function actionMyPlans($status_type = InstapPlanPool::STATUS_TYPE_ALL, $page = 0, $pageSize = self::MAX_ROW_PER_PAGE) {
        ...
        $models = UserPlan::find()->active()->planStatusInArr($status_type_arr)->andWhere(['user_plan.user_id' => $user_id])->orderBy(['created_at'=>SORT_DESC])->limit($limit)->offset($offset)->all();
        //display different object structure.
        $d = UserPlan::toObjectArray($models);
        Yii::$app->api->sendSuccessResponse($d);
    }
    //api for app after re-skinning
    //groupby 
    public function actionMyPlansByCategory($status_type = InstapPlanPool::STATUS_TYPE_ALL, $category = InstapPlan::ALL_CATEGORY, $tier = InstapPlan::ALL_TIER, $page = 0, $pageSize = self::MAX_ROW_PER_PAGE) {
       ...
       if(isset($_GET['tier'])) { 
            if(!in_array($_GET['tier'], array_keys(InstapPlan::allPlanTier()))) {
               throw new CustomHttpException(Utility::jsonifyError("category", "Invalid tier."), CustomHttpException::BAD_REQUEST);
           }
       }
        $limit = ($pageSize > self::MAX_ROW_PER_PAGE) ? self::MAX_ROW_PER_PAGE : $pageSize; //page size
        $offset = $page * $limit;
        $user_id = Yii::$app->user->id;
        $status_type_arr = InstapPlanPool::planStatusType($status_type);
        $models = UserPlan::find()->active()->planStatusInArr($status_type_arr)->byCategory($category)->byTier($tier)->andWhere(['user_plan.user_id' => $user_id])->orderBy(['created_at'=>SORT_DESC])->limit($limit)->offset($offset)->all();
        //display different object structure.
        $d = UserPlan::categorizeByTier($models);
    ...
    public function actionMyPlanDetails() {
        ...
        if(!empty($user_plan_id_arr)) {
            ...
            $str =  Utility::jsonifyError("plan_pool_id", Yii::t("common","Not authorized to get plan detail."));
    ...
    //this endpoint doesn't serve as complete registration.
    public function actionRegisterPartially() {
        if (!isset($_POST['json'])) {
            throw new CustomHttpException(Utility::jsonifyError("json", Yii::t('common',"No json data.")), CustomHttpException::BAD_REQUEST);
        }
        $complete = false;
        ...
        $form->scenario = RegisterPlanForm::SCENARIO_PARTIAL;

        if ($form->validate() && $pool = $form->registerPartially()) {
            if ($pool) {
                $data = [];
                $data['plan_pool_id'] = $pool->id;
                $data['policy_number'] = $pool->policy_number;
